    ...
    <Layout>
      {!getWorker._id ? (
        ...
        <>
          <div className="font-bold flex flex-col justify-center items-center">
            <h2 className="text-3xl text-titleFont mb-6">
              Detalle del profesional seleccionado
            </h2>
            <div className="justify-center items-center mt-5 flex flex-col flex-wrap">
              <div
                key={getWorker._id}
                className="flex  bg-neutral-300 p-5 mb-10 mr-5 rounded-xl duration-200 hover:scale-105 w-[35rem]"
              >
                <Image
                  key={getWorker._id}
                  src={getWorker.profilepic}
                  className="w-40 h-40"
                  width={100}
                  height={100}
                  alt="pics"
      ...
      <div className="font-bold flex flex-col justify-center items-center">
        <h2 className="text-3xl text-titleFont text-blue-500 mb-6 flex items-center">
          ...
          <IoIosStarOutline className="text-gray-400" />
          <IoIosStarOutline className="text-gray-400" />
          Opiniones del servicio <IoIosStarOutline className="text-gray-400" />
          <IoIosStarOutline className="text-gray-400" />
          <IoIosStarOutline className="text-gray-400" />
        </h2>
        <div className="justify-center items-center mt-5 flex flex-wrap">
          {jobsReviews.map((item, index) =>
            item.statejob === "Done" ? (
              <div
                key={index}
                className="card bg-neutral-300 p-5 mb-10 mr-5 rounded-xl duration-200 hover:scale-105 w-[35rem] border border-blue-500"
              >
                <p>
                  <strong>Empleador:</strong> {item.employer[0].name}
                </p>
                <div className="flex items-center">
                  <p>
                    <strong>Rating:</strong> {item.ratingWorker}
                  </p>
                ...
                <p>
                  <strong>Comentario:</strong> {item.reviewWorker}
                </p>
              </div>
            ) : null
          )}
